...
- `providers/` — concrete providers. `builtin.py` = **`BuiltinMemoryProvider`** (§1.3),
  the lean read/seam wrapper over the §1.2 store (§1.4 adds candidate/semantic here).

Deferred: the governed model-facing `memory` **write tool** + agent-surface tool
injection = §1.5; the embedded vector store + entity providers = §1.4; real threat
patterns + `StreamingContextScrubber` + real pre-compression wiring = §1.6;
`CompositeMemoryProvider` multi-provider merging = Phase 2.
...
- `providers/` — 具体 provider。`builtin.py` = **`BuiltinMemoryProvider`**（§1.3），§1.2 存储之上的精简读/接缝包装
  （§1.4 在此加入候选/语义）。

推迟：受治理的面向模型 `memory` **写工具** + agent 表面工具注入 = §1.5；嵌入式向量库 + 实体 provider = §1.4；
真实威胁模式 + `StreamingContextScrubber` + 真实压缩前接线 = §1.6；`CompositeMemoryProvider` 多 provider 合并 = 第二阶段。
