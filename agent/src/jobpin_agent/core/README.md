...
- `session_store.py` — SQLite session store with branch/reset.
- `config.py` — env-based `CoreConfig`.
- `agent_loop.py` — the synchronous turn loop (`Agent`, `TurnResult`).
...
- `session_store.py` — 带 branch/reset 的 SQLite 会话存储。
- `config.py` — 基于环境变量的 `CoreConfig`。
- `agent_loop.py` — 同步回合循环（`Agent`、`TurnResult`）。
