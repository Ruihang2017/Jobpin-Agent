...
```bash
cd agent
python -m pytest -q          # 28 passed, 1 skipped (OpenAI integration; opt-in)
...
**Watch what happens:** every turn the `Tracer` records each step with full detail — the system prompt
+ messages sent, the model response, `latency_ms`, and token `usage`, plus each tool's `arguments`
and `result`. `examples/chat.py` prints a per-turn step + token summary and writes the full JSONL
trace to `agent/traces/latest.jsonl`; type `/trace` to dump every step's prompt/response/tool IO.
