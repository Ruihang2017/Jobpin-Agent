...
```bash
cd agent
python -m pytest -q          # 28 passed, 1 skipped（OpenAI 集成测试；可选）
...
**观察运行过程：** 每个回合 `Tracer` 都以完整细节记录每一步——发送的系统提示与消息、模型响应、`latency_ms`
与 token `usage`，以及每个工具的 `arguments` 与 `result`。`examples/chat.py` 打印每回合的步骤与 token 概要，并把
完整 JSONL 追踪写入 `agent/traces/latest.jsonl`；输入 `/trace` 可打印每一步的提示/响应/工具 IO。
