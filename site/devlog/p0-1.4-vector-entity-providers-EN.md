...
**Why two stores — structured (relational) vs semantic (vector).** This is the question the whole layer
exists to answer, so it leads here. The relational `candidates` store holds the **discrete, enumerable
fields** you filter/sort/match exactly (`skills[]`, `years`, `location`, `work_rights`, `consent`) —
answered by SQL `WHERE`/`=`/`IN`. The vector store holds the **free-form text that doesn't fit columns**
(résumé paragraphs, interview feedback, JDs, interaction history) **+ its embedding** — answered by
**semantic nearest-neighbour**. They are complementary: relational = hard must-haves + RBAC; vector =
semantic ranking *within* the allowed set (the filter-before-NN path). A vector record stores the chunk
**`text`** (to render the citation) + `struct_ref`/`memory_key` (to join back to the structured row) —
**nothing the columns already hold**.

*Why the vector store earns its place* (PRD M1 / F1.3 — "misses paraphrased capabilities"): a résumé
reading *"architected a globally-distributed, eventually-consistent ledger at 2M tx/s; mentored four
engineers"* has only `skills=["go","kafka"]` in the relational store. A search for *"distributed systems
at scale and team mentoring"* **misses** that candidate on columns/keywords but **finds** them by
meaning — there is no finite schema for "every capability a person might have", so the prose + its vector
are the only place that signal lives.

*Honest caveat — what §1.4 does NOT yet demonstrate:* with the **fake `hashing_embedder` (lexical overlap
only, not semantics)** and short demo chunks, the vector store isn't yet pulling weight the relational
store couldn't — it matches `ledger`↔`ledger` but **not** `"distributed systems"`↔`"globally-distributed
ledger"`. The `examples/recall_demo.py` query therefore deliberately shares *words* with the résumé prose
(and the prose, not the columns, is where they live — her skills are `go`/`kafka`); a real **semantic**
match needs a real embedder (BGE/OpenAI behind `EmbedFn`, **config/§1.12**) and real résumé **parsing**
into rich chunks (**§1.11/§1.15**). §1.4 delivered the *capability + plumbing*, not the semantic payoff.
...
```
`recall_demo.py` output: `{"recall_in_prompt": true, "has_citation": true, "fenced": true, "answer": "Ada Lovelace — globally-distributed ledger + mentoring — is the strongest match."}` — the query (`"who built a globally-distributed ledger and mentored engineers?"`) hits her résumé **prose** (vector store), which her structured columns (`skills=["go","kafka"]`) never held.
