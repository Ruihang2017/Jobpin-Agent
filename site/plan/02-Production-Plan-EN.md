...
- **Key relaxation**: Hermes enforces "only one external Provider at a time" (`add_provider` rejects a second non-builtin). HR needs multiple dedicated Providers coexisting — **this phase first runs on Hermes's single-Provider model**, explicitly deferring "multi-provider merging (`CompositeMemoryProvider`)" to Phase 2 (per the simplicity principle of Section 13.1 of the PRD; trigger signal = introducing employee memory M4). This phase **reserves a routing abstraction** inside the Manager but does not enable merging.
...
| wedged provider does not block exit | A provider's `sync_turn` simulates blocking for hundreds of seconds | The main turn is not blocked; `shutdown_all` completes bounded draining within `_SYNC_DRAIN_TIMEOUT_S` (5.0s) (the worker is a daemon) |
...
- **Session-switch semantics**: `on_session_switch(new_session_id, parent_session_id, reset, rewound)` fires on `/resume`, `/branch`, `/reset`, and compression re-continuation; the Provider refreshes its per-session cache accordingly, ensuring writes land in the correct session record — in the HR scenario, "one recruitment loop continuing across sessions" depends heavily on this semantics. This phase reserves a provider routing table inside the Manager keyed on the `entity_type` segment of `memory_key`, but **does not enable merging** (`CompositeMemoryProvider` is left for Phase 2).

**Exit Criteria**:
- The built-in Provider completes the "prefetch → turn → sync → queue_prefetch" loop through the Manager; assert that persistence is visible after `flush_pending`.
- Inject a slow / wedged provider (simulating blocking for hundreds of seconds): the main turn is not blocked, and the process exits within `_SYNC_DRAIN_TIMEOUT_S` after completing draining.
...
**Implementation Notes (How)**:
- **Governance is a first-class citizen of the write path**: make "provenance + lawful-basis labels" a **pre-check** of `MemoryStore.add/replace` and the Provider's `sync_turn` — missing labels are rejected outright, reusing the existing "reject invalid writes outright" skeleton from 1.2 (Hermes already takes this stance against empty content / injection / drift).
- **The honest boundary of erasure**: make clear that "erasure = immediate in the live store + backups age out", and do not promise GDPR-style immediate full wiping — this is a physical limitation of local-first + file backups and must be stated truthfully in the product UI and compliance documents (Section 9.5 of the PRD).
...
| File-backed memory participates | The file-backed store exposes `on_pre_compress` via the minimal Provider | File-backed key facts are also extracted and merged (fixing "MemoryStore is not a Provider and has no hook") |
