...
| 文件型记忆参与（接缝层面） | 文件型 store 经最小 Provider 暴露 `on_pre_compress` 并由 Manager 聚合驱动 | 文件型 provider **参与生命周期**（修复"MemoryStore 非 Provider 无钩子"）。它返回空，因为策展 Org/Recruiter 记忆是**人工编辑、并非由对话派生**——无可抽取之物；从*对话*抽取关键事实是抽象式摘要器之职，推迟到 §1.11（用 LLM）。§1.6 交付**接线**（捕获 + 并入 + 门控持久化），而非对话内容抽取。 |
...
**实现要点（How）**：强化点 ② 的集成测试（上表"压缩后仍可召回"）是最高价值交付，直接回应 PRD 第 9.1 / 8.3 节"重要更正"；移植威胁库**连带核查传递依赖许可证**（MIT 移植卫生）；**不松动 scope 哲学**——保留"锚定 C2 特定词汇 / 明确攻击行为，而非锚定命令式英语"，不因 HR 文本常见"you must / please ensure"就放宽到误报满天飞，记忆写入固定 `strict`。压缩**摘要器是注入接缝**（`summarize_fn`）：§1.6 交付**确定性、保真**默认，仅约束消息*条数*（使循环不再触发）而**非** token 占用——真正控制上下文预算的抽象式/有损 LLM 摘要器落在 §1.11/配置；§1.6 的价值（捕获→并入接线抢救有损摘要会丢弃的事实）在测试套件中用有损 `summarize_fn` 证明。
...
**What（契约）**：本地数据与记忆的静态加密、访问控制、密钥 / secret 管理、（企业场景）SSO——本地优先下，"安全"既是隐私卖点，也是 NDB 下的**严重损害缓解措施**（强静态加密在泄露评估中可作为不构成"严重损害"的有力因素；它**不是**法定安全港——见 PRD §11.5"不可夸大为可规避"）。

> **Phase 0 范围决策（2026-06-30）**：本工作流中，Phase 0 仅交付核心资产与合规卖点所**依赖**、且事后改造（数据 / 召回层）代价高昂的两项——**静态加密 + OS keystore 密钥托管** 与 **RBAC/ABAC 引擎**（被 §1.5 记忆召回过滤器复用）。其余——**字段级敏感字段加密、secret 管理（存储 + 轮换）、SSO、更新包签名 / 完整性校验**——**推迟到后续阶段（Phase 1+）**，各自在触发条件到来时再建（出现真实客户 BYO-key / 多个连接器凭据 → secrets；首个企业试点 → SSO；自动更新 / 分发加固 → 签名；数据层加固阶段 → 字段级加密）。Phase 0 退出门槛（§1.16）**不变**：它本就只对上述两项基础把关。

**范围（Scope）**：

*Phase 0（不可删减——基础性、难以事后改造）：*
- **静态加密**：本地数据库与记忆文件静态加密；密钥由 OS keystore 管理（Windows DPAPI / macOS Keychain）；裸盘读取仅得到密文。
- **RBAC + ABAC**：按 org / 团队 / 角色 / 敏感度；最小权限。与 §1.5 记忆召回 RBAC 为**同一引擎**（单一事实来源——无权限模型漂移）。

*推迟到后续阶段（触发条件到来时再建）：*
- **字段级加密**：对单独敏感字段做按列加密（在静态加密层之上的纵深防御）——在数据层加固阶段、待静态加密层就绪且 schema 标注敏感列后再做。
- **secret 管理**：模型 API 密钥（含客户 BYO-key）、连接器凭据——安全存储 + 轮换。Phase 0 过渡期沿用现有被 gitignore 的 `.env` / OS-keystore；专用存储 + 轮换在 Phase 1（出现真实客户 BYO-key / 多个连接器凭据时）落地。
- **SSO（企业场景）**：OIDC / SAML 接入骨架——SMB 默认本地账户；待首个企业试点需要时再做。
- **本地应用完整性**：更新包签名与完整性校验骨架——随自动更新 / 分发加固推进（与 1.13 打包工作配套）。
...
- [ ] 对上述两项基础的威胁建模 v1 初评通过记录（与 1.14 架构文档对接）。

*推迟（后续阶段）：*
- [ ] `security/encryption`（字段级）：敏感字段按列加密。
- [ ] `security/secrets`：secret 存储 + 轮换。
- [ ] `security/sso`：OIDC / SAML 接入骨架。
- [ ] `security/integrity`：更新包签名 + 完整性校验骨架。

**实现要点（How）**：安全基线 RBAC/ABAC 与 1.5 记忆召回 RBAC **同源**（记忆 prefetch 过滤器直接复用本引擎判定，避免两套权限模型漂移）；主密钥不落盘明文、经 DPAPI / Keychain 托管，裸盘读取得到密文。

**退出标准（Exit）**（不变——本就只对 Phase 0 两项基础把关）：
...
> **Phase 0 范围决策（2026-06-30）**：Phase 0 交付集成**框架**——连接器 SDK + 反腐层、MCP **工具暴露骨架**（连接器操作作为既有 `ToolRegistry` 中的 `ToolSpec`；真实 MCP **服务端/传输**为 §1.12 spike 5）、**"完全本地"开关**与**逐次出站审计**——并以**伪只读 ATS 连接器**+ 契约测试验证（无 PII、无网络）。**真实 ATS/HRIS 实时连接推迟**，直到**同时**具备真实凭据**且** §1.11 脱敏管线就绪：在无脱敏的情况下向真实外部系统拉取/推送真实候选人 PII 会违反本地优先 / APP 8（"无脱敏不出站"）。此举对齐 §1.1（已交付 `ModelProvider` + `FakeProvider`；仅因账号存在才有实时 `OpenAIProvider`）。双向同步 / 多连接器留待 Phase 1–2。"完全本地 ⇒ 0 出站"保证（退出标准 2）现已交付。
...
**退出标准（Exit）**：
- 从一个 ATS/HRIS 连接器只读拉取数据（**Phase 0**：以**伪**连接器做契约测试；**实时**真实 ATS 连接以真实凭据 **+** §1.11 脱敏管线为前置——见范围决策）→ 经反腐层翻译为规范实体 → 入本地库，全程可关闭。
...
**范围（Scope）**：
- **本地应用打包 / 分发**：一键安装包（Windows 优先）+ 自动更新雏形。*（更新包签名 / 完整性校验**随 §1.9 `security/integrity` 一并推迟**到后续阶段——见 §1.9 的 Phase 0 范围决策；Phase 0 交付安装 + 自动更新雏形，**不含**签名要求。）*
...
**退出标准（Exit）**：
- 本地应用可**一键安装并自动更新**（雏形）。*（签名 / 完整性校验随 §1.9 一并推迟——见范围——故**不是** Phase 0 退出要求。）*
