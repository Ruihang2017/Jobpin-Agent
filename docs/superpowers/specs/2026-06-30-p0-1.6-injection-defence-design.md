...
### Seam wiring (composition + fail-safe defaults)
- Curated store: `build_memory_backend` **defaults** `scan_entry` to `first_threat_message(·, "strict")`
  (overridable) — memory writes use the most aggressive scope.
- Candidate + Semantic providers (the memory sinks for résumé/KB chunks): their `scan_entry` **defaults**
  to `first_threat_message(·, "context")` (fail-safe — résumé/KB text is untrusted; overridable to disable).
  *(Post-review fix: triple-review flagged that leaving these pass-through-by-default would fail open; the
  fail-safe default mirrors the curated-store precedent.)* The seams already existed (§1.4); §1.6 makes the
  default real on every untrusted-text sink — both the context door and the memory sinks.
