...
| `_CORE_TOOL_NAMES` reserves the governed tool name | `manager.py:64` | the model-facing `memory` write tool, born governed |
...
1. **Enforce governance in the governed tool's write path, leave `MemoryStore` unmodified.** The store's
   ...
   and it keeps the Hermes port faithful. **Defense-in-depth note:** the entity path *does* use the
   provider `write_gate` (ingest) for the consent check, because that path has no model tool.
