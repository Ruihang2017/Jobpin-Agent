...
- `class AuditStore(conn_or_db_path)`:
  ...
  - `import_transitions(transitions) -> int` — *(implemented signature; cleaner than the earlier
    `(store, instance_id)` sketch — the caller selects the `Transition`s, e.g. `store.transitions_for(id)`)* —
    map §1.7 `Transition`s to `AuditRecord`s (`actor`, `action="transition"`, `target_key=instance_id`,
    `reason=f"{from}->{to}:{trigger}"`, `at_wall=at`, `at_monotonic=0.0` — historical rows lack monotonic).
  - **Thread-safe** (triple-review fix): `CanonicalStore` opens the connection `check_same_thread=False`
    and a shared `threading.Lock` serialises all connection access, so the §1.3 background worker can record
    the read-path audit (the §1.5 deferral) — delivering the property the §1.5 docstring promised §1.8.
  - **One-shot import** (triple-review fix): the import adapters are a non-idempotent snapshot — re-importing
    duplicates rows (no dedup); the canonical table is the unified query entry point *after* a reconciliation
    import. A live/incremental view (or rewiring the emitters) is a Phase-2 consolidation.

### `data/store.py` read surface (post-review)
`CanonicalStore` exposes `get_*` for every entity (candidate / job / application / interview / **consent** —
the lawful-basis anchor / org / user) + `consents_for_candidate(candidate_id)` and
`memory_records_under(prefix)` — the latter two back the data-subject query (`Candidate → Consent` +
`Candidate → MemoryRecord` by `memory_key` prefix). *(Added in the triple-review to close the read-surface
gap; the full APP-12 access portal is F3.6.)*
